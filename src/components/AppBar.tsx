...
const PcNavigator = (function ({ scrollLeft }: { scrollLeft: number }) {
    ...
    return <>
        <div className="gnb only-pc container" style={xPositionStyle}>
            <Link to="/" className="logo" onMouseEnter={setShowSubpageNavigatorTrue}><img src={Logo} alt="logo" /></Link>
            <ul>
                {
                    routes.filter(({ showInNav }) => (showInNav)).map(({ name, path }, key) => (
                        <li key={key}>
                            <NavLink
                                className={`nav-item ${lnbNavHoveringGnbPath === path ? "active" : ""}`}
                                to={path}
                                onMouseEnter={setShowSubpageNavigatorTrue}>
                                {name}<div className="underline" />
                            </NavLink>
                        </li>
                    ))
                }
        ...
        </div>
        {
            ...
            <div className="lnb only-pc">
                <div className="container" style={xPositionStyle}>
                    <SubRoutesLinks onHoveringGnbChanged={(gnbPath) => { setInbNavHoveringGnbPath(gnbPath) }} />
                </div>
            </div>
        }
    </>
...
const SubRoutesLinks = withRouter(
    ({ location, onHoveringGnbChanged }: SubRoutesLinksPropsType) => {
        return <>
            {routes.filter(({ showInNav }) => (showInNav)).map(({ subRoutes, path: gnbPath, korName }, key) => (
                <div className="lnb-item" key={key} onMouseEnter={() => { onHoveringGnbChanged(gnbPath) }} onMouseLeave={() => { onHoveringGnbChanged(null) }}>
                    <p>{korName}</p>
                    <ul>
                        {
                            subRoutes && subRoutes.map(({ name, hash, path: lnbPath }, key) => (
                                <li key={key}>
                                    <NavLink
                                        isActive={(match, location) => {
                                            ...
                                            return (!hash || hash === location.hash) &&
                                                location.pathname === `${gnbPath}${lnbPath || ""}`;
                                        }}
                                        className="nav-item"
                                        replace={location.pathname === gnbPath}
                                        to={{ pathname: `${gnbPath}${lnbPath || ""}`, hash }}
...
function MobileMenu() {
    ...
    return <div>
        <div className="top only-mobile container">
            ...
            <img className="menu-button" src={menuOpened ? IcClose : IcMenu} alt={menuOpened ? "hamburger-close" : "hamburger-open"} onClick={() => { setMenuOpened(!menuOpened) }} />
        </div>
        {
            menuOpened &&
                ...
                <ul>
                    {
                        routes.filter(({ showInNav }) => (showInNav)).map(({ name, path }, key) => (
                            <li key={key}>
                                <Link className="nav-item" to={path} onClick={() => { setMenuOpened(false) }}>{name}</Link>
                            </li>
                        ))
                    }
                </ul>
