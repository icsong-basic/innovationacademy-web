...
export default function Footer({ }: Props) {
    ...
    return (<>
            ...
            <div className="container">
                ...
                <div className="row only-pc">
                    <div className="col md-3">
                        <p className="privacy-policy cursor-pointer" onClick={() => { setOpenPrivacyPolicy(true) }}>개인정보처리방침</p>
                        <p className="copyright">2019 Innovation Academy.<span className="only-pc"> All Rights reserved.</span></p>
                    </div>
                    ...
                    <div className="col md-3 link-container">
                        <div className="site-select">
                            <LinkSelectBox text="프로젝트" links={programLinks}>
                            </LinkSelectBox>
                            <LinkSelectBox text="관련 사이트" links={relatedSiteLinks}>
                            </LinkSelectBox>
                        </div>
                ...
                <div className="only-mobile">
                    <div className="row">
                        <div className="col sm-6">
                            <p className="privacy-policy" onClick={() => { setOpenPrivacyPolicy(true) }} >개인정보처리방침</p>
                            <p className="copyright">2019 Innovation Academy.<span className="only-pc"> All Rights reserved.</span></p>
                        </div>
                    </div>
                    <div className="row">
        ...
        {
            openPrivacyPolicy ?
                <PrivacyPolicy onCloseRequest={() => { setOpenPrivacyPolicy(false) }} /> : undefined
        }
    </>)
}
...
function LinkSelectBox({ text, links, children }: { text: string, links: { name: string, url: string }[], children?: any }) {
    ...
    return <div>
        <p onClick={() => { setMenuOpened(!menuOpened) }} className={menuOpened ? "minus" : "plus"}>{text}<i /></p>
        <ul className={"menu " + (menuOpened ? "" : "hidden")}>
            {
                links.map(({ name, url }, key) => (
                    <li key={key}><a href={url} target="_blank" rel="noopener noreferrer">{name}</a></li>
                ))
            }
        </ul>
        {children}
